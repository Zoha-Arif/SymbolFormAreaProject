...
%generate column of tracts of interest ids
tractIDs = {'RNFA', 'LNFA', 'RVWFA', 'LVWFA'}; 

varNames = {'Subject', 'Tract', 'k', 'kmeans', 'silhouette', 'idx', 'data'}; 
varTypes = ["double", "char", "double", "cell", "double", "cell", "cell"]; 
...
for i = 1:length(kValues)
    ...
    for t = 1:length(tractIDs)
        ...
        tbl = table(xyz.subjectID, xyz.(tractIDs{t}), 'VariableNames', {'Subject', 'Data'}); 
        ...
        coordTBL = table(resultSubjectID, coordArray(:,1), coordArray(:,2), coordArray(:,3), 'VariableNames', {'Subject', 'x_coords'... 
            'y_coords', 'z_coords'});
    ...
    for g = 1:length(tractIDs)
        ...
        if ~(isempty(subIDX))
            ...
            for v = 1:k
                ...
                clusterData = data(idx == v, :); 
                scatter3(clusterData(:, 1), clusterData(:,2), clusterData(:,3), [], colors(v, :), 'filled'); 
            end
            ...
            mainpath = '/Volumes/LANDLAB/projects/sfa/supportFiles/final-plots-kmeans-var';
            filename = strjoin(['sil_kmeans_' string(subIDs(s)) '_' tractIDs(g)]); % Define a filename for each plot
...
for a = 1:length(tractIDs)
    ...
    %export figure as a png file
    mainpath = '/Volumes/LANDLAB/projects/sfa/supportFiles/final-plots-kmeans-var';
    filename = strjoin(['sil_Math_', tractIDs(a)]); % Define a filename for each plot
    fullFilePath = fullfile(mainpath, filename); % Create the full file path
    saveas(nfig, fullFilePath, 'png'); % Save the figure as a PNG file
...
for a = 1:length(tractIDs)
    ...
    %export figure as a png file
    mainpath = '/Volumes/LANDLAB/projects/sfa/supportFiles/final-plots-kmeans-var';
    filename = strjoin(['sil_Read_', tractIDs(a)]); % Define a filename for each plot
    fullFilePath = fullfile(mainpath, filename); % Create the full file path
    saveas(nfig, fullFilePath, 'png'); % Save the figure as a PNG file
